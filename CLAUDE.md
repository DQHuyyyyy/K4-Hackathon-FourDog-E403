...
> Cập nhật lần cuối: 2026-07-30 tối, sau khi xong "tổng kết cuối buổi + bộ thẻ ôn" và sửa lỗi badge `?`.
...
**Sản phẩm:** **Catch Me Up** — trợ lý ngữ cảnh nằm cạnh slide trong VLearn. Học viên bị phân tâm 2–3 phút, lỡ 4–5 trang; bấm một nút, AI đọc các trang đã trôi qua, **quyết định trang nào chứa ý chính vs trang phụ**, trả về bullet kèm số trang bấm được để nhảy tới.

**Lát cắt một câu (Canvas dòng 5):**
...
| Route | Kiểu | Việc |
|---|---|---|
| `POST /api/chat` | SSE stream | Chat tự do; server inject nội dung trang đang xem + tiêu đề 2 trang lân cận + đoạn bôi đen |
| `POST /api/catchup` | JSON Schema | `{from,to}` → `{bullets, skipped, meta}` |
...
**Nội dung slide:** `demo/pages.json` — 16 trang đúc từ transcript Day 02 trong data pack, giữ nhãn `/ 76` như tài liệu thật trên VLearn. Trang 3 khớp nguyên văn slide thật. **Trang 4 là "Agenda"** — cố ý nằm trong khoảng lỡ mặc định (4–7) để demo được việc AI nhận ra trang phụ và bỏ qua.

**Hàm chính trong `app.js`:** `renderPages()` · `setCur()` · `goTo()` · `sendChat()` (streaming) · `catchUp()`/`showSummary()` · `askStruggle()` · `tickDwell()`/`arm()`/`disarm()` · `wrapUp()`/`showWrapup()`/`exportWrapup()` · `openDeck()`/`startDeck()`/`renderCard()`/`flipCard()`/`nextCard()`/`deckDone()` · `highlightSelection()`/`wrapRange()` · `wirePen()` · `resetDemo()`.

**State:** `cur` (trang đang xem) · `presenter` · `A[n].ops` (annotation từng trang) · `chatMsgs` · `struggled` (Set trang đã hỏi) · `dwellMs` · `wrapData` (bản tổng kết gần nhất) · `deck`/`deckI`/`deckShaky` (bộ thẻ đang ôn).
...
**Quyết định 1 — `/api/catchup`: "trang này là ý chính hay trang phụ?"**
Đây là lát cắt chính và là chỗ ăn điểm R4. Nó **không phải** "sinh tóm tắt" — sinh tóm tắt thì không đo được. Quyết định chọn/bỏ trang thì so được với ground truth → ra precision/recall.

**Cùng quyết định 1, phạm vi rộng hơn — `/api/wrapup`** (thêm 30/07 tối)
Tổng kết cuối buổi hỏi model **đúng câu hỏi đó**, chỉ khác là trên cả tài liệu thay vì 4–5 trang bị lỡ. Vì cùng loại quyết định nên **dùng chung golden set với `catchup`, không phát sinh bộ eval thứ hai** — đây là lý do tính năng này không phá nguyên tắc "một lát cắt" ở §12.
Khác biệt duy nhất ở đầu ra: `catchup` trả một bullet mỗi trang, `wrapup` **gom** các trang cùng một ý thành một takeaway `pages:[7,8,9]` — nối lại mạch xuyên trang.
...
Thêm ngày 30/07 cùng tính năng phát hiện dừng lâu. ⚠️ **Đây mới là chỗ thật sự phát sinh quyết định thứ hai** — nếu đưa vào chấm R4 thì cần golden set riêng. Muốn giữ đúng "một lát cắt" thì để nó là tính năng phụ trợ và chỉ đo `catchup` + `wrapup`.

---

## 7. Bằng chứng đã có (quan trọng cho CP3 + R4)

`eval/runs.jsonl` — **54 dòng**, trong đó **15 lượt gọi OpenAI thật thành công** (4 `catchup` · 6 `chat` · 3 `hints` · 1 `wrapup` · 1 `deck`), phần còn lại là lượt qua mock khi test và các dòng lỗi.
...
⚠️ **Chuyện quota:** có đúng một dòng lỗi `quota` lúc 10:19, nhưng **7 lượt thật sau đó (10:24→10:30) đều thành công** — nên cảnh báo "hết credit" trong bản CLAUDE.md cũ là sai. Dù vậy vẫn nên **kiểm số dư trước khi pitch**.
...
### Tutor
- **Chat streaming AI thật**, chữ hiện dần, badge `model · giây · token` dưới mỗi câu trả lời
...
- `⏸ Mô phỏng rời đi 3 phút` → badge coral "Giảng viên đang ở trang 7"
- Tutor đẩy alert "Bạn đã lỡ 4 trang" + nút `⚡ Catch me up (4–7)`
- Gọi `/api/catchup` thật → bullet có chip số trang + chip **"Bỏ trang phụ: 4"** + badge model/latency/token
- Bullet có "chưa chắc" → chip đổi màu coral (đúng Canvas dòng 6)
- Bấm bullet → nhảy tới trang, trang nháy viền xanh, bullet mờ đi
- `✓ Đã bắt kịp` → về trạng thái theo dõi · `↺ Reset` chạy lại

### Tổng kết cuối buổi + bộ thẻ ôn (mới, 30/07 tối)
...
- Gọi `/api/wrapup` → bong bóng tổng kết: ý **gom theo phần** (`PHẦN 2 · TÌM VẤN ĐỀ`), mỗi ý kèm **nhiều chip số trang**, chip "Bỏ trang phụ", badge model/latency/token
...
- Reset khi đổi trang; tạm dừng khi tab ẩn (alt-tab > 30s thì tính lại); **tắt hẳn** khi đang mô phỏng rời đi
...
**Vì sao hoãn:** Evidence Gate #4 chấm "lát cắt đủ sắc — 1 người dùng · 1 công việc · 1 quyết định AI · 1 kết quả". Mỗi tính năng mới là một quyết định AI mới cần golden set riêng cho R4. `CP2.md` §8 nguyên tắc 1 đã chốt: CP2–CP6 chỉ demo Catch Me Up, phần còn lại gói làm roadmap trong pitch.
...
| `/api/catchup` happy path | ✅ qua mock — parse JSON Schema, picked [5,6,7] skipped [4] |
...
1. **Đặt lại `.env` ở gốc repo** (§3) — không có thì server không chạy được, và hai route mới chưa từng gọi model thật.
2. **Chạy thật một lượt `wrapup` + `deck`** để có bằng chứng AI thật cho tính năng mới, đồng thời kiểm số dư OpenAI trước khi pitch.
...
5. **Dựng golden set** — 3–5 đoạn 4–5 trang, đánh dấu ground truth, chạy AI, tính precision/recall từ `picked`/`skipped` trong `runs.jsonl`. Cùng bộ này chấm được cả `catchup` lẫn `wrapup` (§6). R4 15đ đang bỏ trống.
...
1. **Một lát cắt duy nhất** — mọi tính năng khác là roadmap trong pitch, không build. Ngoại lệ duy nhất được chấp nhận: tính năng **không thêm quyết định AI mới** (như tổng kết cuối buổi, xem §6)
